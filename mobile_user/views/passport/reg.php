...
$(function() {
	...
	$('#btn-phonecode').click(function() {
		...
		if(!is_mobile(mobile)) {
			...
			$.post("http://app.diabetes.com.cn/guanhuai/index.php/passport/logsms",//'./logsms',
				{
					mobile: mobile,
					err:"<?php echo base_url('index.php/passport/logsms'); ?>",
					iserr:"2"
				}, function(data) {},'text');
						
			$.post("<?php echo base_url('index.php/passport/phonecode'); ?>",//'./phonecode',
				{
					mobile: mobile,
					pic:pic,
					...
					if(data == 1) {
						...
						$.post("<?php echo base_url('index.php/passport/logsms'); ?>",//'./logsms',
						{
							mobile: mobile,
							err:data,
							iserr:"0"
						...
						$.post("<?php echo base_url('index.php/passport/refresh'); ?>",//'./logsms',
						{
						}, function(data) {
						...
						luiji_alert('error', '验证码不能频繁发送给同一手机号！');
						nn_enableButton(_btn, '获取验证码');
					} else if(data == 4) {
						luiji_alert('error', '手机号不能为空！');
						nn_enableButton(_btn, '获取验证码');
					} else if(data == 5) {
	...
	$('#submit').click(function() {
		...
		// } else if(!is_phonecode(phonecode)) {
			...
			//$.post('./reg',
			$.post("<?php echo base_url('index.php/passport/reg'); ?>",
				{
					mobile: mobile,
					username: username,
					password: password,
