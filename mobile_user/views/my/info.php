...
<script src="<?php echo site_url(); ?>public/js/areas.js"></script>
...
$(function() {
	...
	var
		...
		provinces_options = '',
		current_province = '<?php echo $user['province']; ?>',
		current_city = '<?php echo $user['city']; ?>'
	;
...
<div id="editpwd" class="gh-center">
	<div class="login-form">
		<div class="form-item">
			...
			if($user['thumb']) {
			?>
			<div class="uploader-preview" id="thumb-box" style="display:inline-block;"><img src="<?php echo nn_fix_uploadurl($user['thumb']); ?>" width="100%"/></div>
			<?php
			} else {
			?>
			...
			<div class="uploader-box">
				...
				<input type="hidden" id="thumb" name="thumb" value="<?php echo $user['thumb']; ?>"/>
			</div>
			<div class="uploader-queue" id="uploader-queue-thumb"></div>
		</div>
		<div class="form-item">
			<label for="username">姓名：</label>
			<input type="text" name="username" id="username" placeholder="输入姓名" value="<?php echo $user['username']; ?>"/>
		</div>
		<div class="form-item">
			<label for="sex">性别：</label>
			<fieldset data-role="controlgroup" data-type="horizontal" style="padding-bottom:16px;">
				<input type="radio" name="sex" class="radio-sex" id="radio-sex-male" value="男" <?php echo ($user['sex'] == '男') ? 'checked="checked"' : ''; ?>/>
				<label for="radio-sex-male">男</label>
				<input type="radio" name="sex" class="radio-sex" id="radio-sex-female" value="女" <?php echo ($user['sex'] == '女') ? 'checked="checked"' : ''; ?>/>
			...
			<fieldset data-role="controlgroup" data-type="horizontal" style="padding-bottom:16px;">
				...
				<select name="birthyear" id="birthyear">
					...
					for($i = date('Y', time()); $i >= 1900; $i--) {
						if($i == $user['birthyear']) echo '<option value="' . $i . '" selected="selected">' . $i . '</option>';
				...
				<select name="birthmonth" id="birthmonth">
					...
					<?php
					for($i = 1; $i < 13; $i++) {
						if($i == $user['birthmonth']) echo '<option value="' . $i . '" selected="selected">' . $i . '</option>';
						else echo '<option value="' . $i . '">' . $i . '</option>';
					}
					?>
				</select>
