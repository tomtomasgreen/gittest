...
class Recordsugar_model extends CI_Model {
	...
	public function get_by_userid_and_type_limit($userid, $type, $time_s , $time_e = 'time' ) {
		...
		// var_dump($time_e);exit;
		$this->db->where( " measuretime > '{$time_s}' and measuretime < '{$time_e}' " );
		$this->db->where("`userid`='{$userid}' AND `type`='{$type}' AND `status`='1'")->order_by('createtime', 'ASC');
		$query = $this->db->get($this->table);
		return $query->result_array();
	}
	
	public function get_by_userid_and_type_sugar($userid , $time_s , $time_e = 'time' ) {
		if($time_s == 'time' && $time_e == 'time'){
			$time_s = mktime(0,0,0,date('m'),date('d')-30,date('Y'));
			$time_e = mktime(0,0,0,date('m'),date('d')+1,date('Y'))-1;
		}else if( $time_s != 'time' && $time_e == 'time' ){
