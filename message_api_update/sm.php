 ...
 * 序列号,请通过亿美销售人员获取
 */
$serialNumber = '9SDK-EMY-0999-JCQQQ';
 ...
 */
$sessionKey = '217B00E8EA86F1900DB90FE72B1A89B4';
...
$mobile = $_POST['mobile'] ? $_POST['mobile'] : $_GET['mobile'];
$yzm = $_POST['yzm'] ? $_POST['yzm'] : $_GET['yzm'];
$sign = $_POST['sign'] ? $_POST['sign'] : $_GET['sign'];
$tmp = sha1($mobile."newnnit@!2O17".$yzm);
if($mobile == null || $mobile == ""){
error_log('ip:'.nn_get_ip().'_time:'.time().'_phone:'.$mobile.'_sign'.$sign.'='.$tmp.'_code:'.$yzm."\r\n", 3, 'errors_moblile_empty.log');
...
function sendSMS($mobile,$yzm)
{
	...
	$statusCode = $client->sendSMS(array(''.$mobile.''),"【糖尿病关怀】您的验证码是：".$yzm);
	$endtime = get_microtime();
	$cost = ($endtime-$starttime)*1000;
	if($statusCode != 0 || $cost>1000) {
		error_log('mobile:'.$mobile.'_code:'.$yzm.'_response:'.$statusCode.'_time:'.$cost.'ms_at:'.time()."\r\n", 3, 'errors.log');
	}
	echo 1;
}
...
function sendVoice()
{
	 ...
	 * $client->sendSMS还有更多可用参数，请参考 Client.php
	 */
	$statusCode = $client->sendVoice(array('13366262521'),"【糖尿病关怀】123456");
	echo "处理状态码:".$statusCode;
}
