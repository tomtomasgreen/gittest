...
class Recordsugar_model extends CI_Model {
	...
	public function get_by_userid_and_type_limit($userid, $type, $limit = 10) {
		$this->db->where("`userid`='{$userid}' AND `type`='{$type}' AND `status`='1'")->order_by('measuretime', 'ASC')->limit($limit);
		$query = $this->db->get($this->table);
		return $query->result_array();
	}

	public function get_by_userid_and_type_since($userid, $type, $start) {
		$this->db->where("`userid`='{$userid}' AND `type`='{$type}' AND `status`='1' AND `measuretime` > '{$start}'")->order_by('measuretime', 'ASC');
		$query = $this->db->get($this->table);
		return $query->result_array();
	}
