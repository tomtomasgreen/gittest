...
class Recordwhr_model extends CI_Model {
	
	private $table;
	
	function __construct() {
		parent::__construct();
		$this->table = 'record_whr';
	}
	...
	public function get_by_userid_limit($userid, $limit = 10) {
		$this->db->where("`userid`='{$userid}' AND `status`='1'")->order_by('measuretime', 'ASC')->limit($limit);
