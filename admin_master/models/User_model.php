...
class User_model extends CI_Model {
	...
	public function get_by_doctorid($doctorid) {
		$this->db->select('user.*,relation.username as uname,relation.userphoto,relation.userdes,relation.createtime as bindtime');
		$this->db->from('user');
	...
	public function get_relation($doctorid, $userid) {
		...
		$this->db->from('user');
		$this->db->join('relation', 'relation.userid = user.id');
		$this->db->where("relation.doctorid='{$doctorid}' AND relation.userid='{$userid}'");
		$query = $this->db->get();
		return $query->row_array();
	}
	...
	{
		if($number == 'all'){
			return $this->db->where(array('status'=>1))->get('user')->result_array();
		}
	}
	
	public function get_all_users($masterid) {
		$this->db->select('user.*');
		$this->db->from('user');
		$this->db->join('relation', 'relation.userid = user.id');
		$this->db->join('department', 'relation.doctorid = department.doctorid');
		$this->db->where("department.masterid='{$masterid}' AND relation.status='1' AND department.status='1'");
